...
Template._cs_datatable.rendered = function() {
    var self = this;
    var container = $(this.find('.cs_container'));
    var table_container = container.find('.cs_table_container');
    _CS_ROW_HEIGHT = table_container.find('.cs_tr').height();
    _CS_NUM_DISPLAYED_ROWS = Math.ceil(container.height() / _CS_ROW_HEIGHT) + 1;
    Meteor.call('_cs_getCollectionSize', this.data.src, function(err, result) {
        if (!err) {
            var totalHeight = result * _CS_ROW_HEIGHT;
            table_container.height(totalHeight + 'px');
...
Template._cs_datatable.helpers({
    setup : function(atts) {
        ...
        var _src = atts.src;
        var _globalOffsetStr = varStr(_src, '_gOffset');
        var _limitStr = varStr(_src, '_limit');
        var _heightStr = varStr(_src, '_height');
        var _sortStr = varStr(_src, '_sort');
        var _localOffsetStr = varStr(_src, '_lOffset');
        Session.set(_globalOffsetStr, atts.offset);
        Session.set(_localOffsetStr, atts.offset);
    ...
    getData : function() {
        ...
        Meteor.subscribe(this.src, offset, limit, sortVar, this.sort);
    },
